package com.ahern.livinghelper.tool.cookbook.adapter;
...
import com.ahern.livinghelper.tool.cookbook.model.CookBookListEntity;
...
//        Glide.with(context).load(list.get(position).getPic()).placeholder(R.mipmap.ic_launcher).into(holder.mCookIconiv);
        ...
        Glide.with(context).load(list.get(position).getPic()).asBitmap().placeholder(R.mipmap.ic_launcher).error(R.mipmap.ic_launcher).centerCrop().diskCacheStrategy(DiskCacheStrategy.ALL)
                .into(new SimpleTarget<Bitmap>(140,140) {
                    @Override
                    public void onResourceReady(Bitmap resource, GlideAnimation<? super Bitmap> glideAnimation) {
                        holder.mCookIconiv.setImageBitmap(resource);
                    }
                });
